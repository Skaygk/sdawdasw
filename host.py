...
async def show_boot_animation():
    if not config.skip_booting:
        stages = [
            f"{Style.BRIGHT}{Fore.LIGHTWHITE_EX}Booting {Fore.RED}Host {Fore.RESET}{Fore.LIGHTWHITE_EX}Tool",
            f"{Fore.RED}25%",
            f"{Fore.YELLOW}50%",
            f"{Fore.LIGHTYELLOW_EX}75%",
            f"{Fore.LIGHTGREEN_EX}99%",
            f"{Fore.LIGHTBLUE_EX}Host Tool ready"
        ]
...
async def leave_all_servers(client):
    ...
    for guild in client.guilds:
        try:
            await guild.leave()
            print(f"{Fore.LIGHTGREEN_EX}Left {Fore.WHITE}{guild.name}")
        except Exception as e:
            print(f"{Fore.RED}Failed to leave {Fore.WHITE}{guild.name}: {e}")
...
async def main_menu(client):
    while True:
        await clear_console()
        print(pyfade.Fade.Horizontal(pyfade.Colors.green_to_white, '''
...
'''))
        ...
        print(f'''{Fore.LIGHTWHITE_EX}                                   Made by {Fore.WHITE}Russo

{Fore.LIGHTGREEN_EX}Logged in as: {Fore.WHITE}{client.user}
